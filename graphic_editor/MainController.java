...
public class MainController {
    ...
    public MainController() {
        ...
        createArcToggle = new ToggleButton("Create Arc");
    ...
    private void handleCreateArcClick(DraggableNode clickedNode) {
        if (selectedNodeForArc == null) {
            ...
            if (selectedNodeForArc != clickedNode) {
                // Створюємо ArcLine для симуляції
                ...
                // Відрізняємо два випадки:
                // - Немає проміжних точок => малюємо звичайний ArcLine (прямий) позаду вузлів
                // - Є проміжні точки => малюємо PolylineArc (ламана) позаду вузлів
                if (tempWayPoints.isEmpty()) {
                    // Звичайна дуга
                    // Додаємо її у pane на початку (індекс 0), щоб була позаду вузлів
                    editorPane.getChildren().add(0, arc);
                    // ArrowHead теж можна додати на початку, але зазвичай arrowHead
                    // з'являється окремо - якщо хочемо, щоб вона не перекривала вузли,
                    // також додамо її на початку:
                    //editorPane.getChildren().add(0, arc.getArrowHead());
                } else {
                    // Ламана
                    PolylineArc polyArc = new PolylineArc(selectedNodeForArc, clickedNode, tempWayPoints);
                    // Додаємо на початку списку, щоб було позаду вузлів
